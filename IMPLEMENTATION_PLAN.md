...
- **Base de Datos:** PostgreSQL con soporte DDL en `init_db.sql` y fallback local a SQLite en desarrollo.
...
6. **`restore`**: Tareas de restauración de backups/sistemas.
7. **`backup`**: Respaldos con soporte de subtareas.
8. **`snapshot`**: Instantáneas de máquinas virtuales y storage.
...
15. **`alta_credencial_especial`**: Requiere `ticket_cliente` y lista de múltiples credenciales asociadas (`persona_propietaria`, `codigo_alfanumerico`).
